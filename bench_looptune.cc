...
void test(int argc, char *argv[])
{
  ...
  std::function<std::tuple<float,float>(int,int,int,int)> scq_test3 = test_enqdeq<scq<bits, 1000>>;
  std::function<std::tuple<float,float>(int,int,int,int)> scq_test4 = test_enqdeq<scq<bits, 10000>>;
  std::function<std::tuple<float,float>(int,int,int,int)> scq_test5 = test_enqdeq<scq<bits, 100000>>;
  std::function<std::tuple<float,float>(int,int,int,int)> scq_test6 = test_enqdeq<scq<bits, 0>>;
  ...
  file << "Threads, 1e3, 1e3_sd, 1e4, 1e4_sd, 1e5, 1e5_sd, 0, 0_sd\n";
  for(int i = 0; i < 8; i++)
  {
    std::cout << threads[i] << " threads\n";

    //SCQ
    std::tuple<float, float> scq_mopss = scq_test3(threads[i], state, OPS, TIMES);
    std::cout << "   1e3 " << std::get<0>(scq_mopss) << "± " << std::get<1>(scq_mopss) << " Mops/sec\n";

    //SCP
    std::tuple<float, float> scp_mopss = scq_test4(threads[i], state, OPS, TIMES);
    std::cout << "   1e4 " << std::get<0>(scp_mopss) << "± " << std::get<1>(scp_mopss) << " Mops/sec\n";

    //BLLOCK1
    std::tuple<float, float> bllock1_mopss = scq_test5(threads[i], state, OPS, TIMES);
    std::cout << "   1e5 " << std::get<0>(bllock1_mopss) << "± " << std::get<1>(bllock1_mopss) << " Mops/sec\n";


    //BLLOCK2
    std::tuple<float, float> bllock2_mopss = scq_test6(threads[i], state, OPS, TIMES);
    std::cout << "   0 " << std::get<0>(bllock2_mopss) << "± " << std::get<1>(bllock2_mopss) << " Mops/sec\n";
    std::cout << "\n";
